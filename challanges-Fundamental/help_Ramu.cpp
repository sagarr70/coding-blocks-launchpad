...
int main(){
    ...
    int min_cost_rikshaw=INT_MAX;
    int mini =0;

   for (int i = 0; i < sizeof(rik)/sizeof(int); i++)
   {  mini = mini+min(rik[i]*c1,c2);
   }
        min_cost_rikshaw = min(mini,c3);
     mini=0;
        int min_cost_of_cab;
   for (int i = 0; i < sizeof(ca)/sizeof(int); i++)
   {
       mini= mini + min((ca[i]*c1),c2);
   }
   min_cost_of_cab = min(mini,c3);
   cout<<min((min_cost_of_cab+min_cost_rikshaw),c4)<<endl;
    n--;
    }
    return 0;
}